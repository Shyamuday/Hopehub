...
if [ -n "${TELEGRAM_USER_BOT_TOKEN_VALUE}${TELEGRAM_DOCTOR_BOT_TOKEN_VALUE}${TELEGRAM_ADMIN_BOT_TOKEN_VALUE}${TELEGRAM_CONTACT_BOT_TOKEN_VALUE}${TELEGRAM_CONFESSION_BOT_TOKEN_VALUE}${TELEGRAM_RULES_BOT_TOKEN_VALUE}" ]; then
  echo "Configuring Telegram bot webhooks..."
  npm run telegram:setup -- --drop-pending
fi

if [ -n "${TELEGRAM_RULES_BOT_TOKEN_VALUE}" ]; then
  echo "Seeding Telegram community settings and automated campaigns..."
  TELEGRAM_RULES_BOT_TOKEN="${TELEGRAM_RULES_BOT_TOKEN_VALUE}" npm run telegram:seed
fi
