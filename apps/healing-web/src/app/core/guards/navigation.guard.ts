...
export class NavigationGuard implements CanActivate {
    ...
    // Define valid service IDs (this could be fetched from a service in a real app)
    private readonly validServiceIds = [
        'breakup-counseling',
        'career-counseling',
        'anxiety-therapy',
        'depression-support',
    ...
    canActivate(
        ...
        if (url.startsWith('/services/') && url !== '/services') {
            ...
            if (!this.validServiceIds.includes(serviceId)) {
                console.warn(`NavigationGuard: Unknown service ID '${serviceId}', redirecting to services list`);
                return this.router.createUrlTree(['/services']);
            }
        }
            ...
            for (const [key, value] of Object.entries(queryParams)) {
                if (typeof value === 'string') {
                    ...
                    if (this.containsSuspiciousContent(value)) {
                        console.warn('NavigationGuard: Suspicious query parameter detected, redirecting to home');
