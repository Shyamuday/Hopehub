...
const TOKEN_KEY = 'hh_patient_token';
...
export class AuthService {
    ...
    async register(credentials: RegisterCredentials): Promise<User> {
        ...
        try {
            const resp = await firstValueFrom(
                this.http.post<ApiAuthResponse>(
        ...
        try {
            await firstValueFrom(
                this.http.post<{ message: string }>(
                    `${this.apiUrl}/auth/patient-forgot-password`,
                    { email: request.email }
                )
            );
        } catch (err) {
    ...
    private loadPreferences(): UserPreferences {
        ...
        try {
            ...
            return raw ? { ...this.getDefaultPreferences(), ...JSON.parse(raw) } : this.getDefaultPreferences();
        } catch {
