...
export class ProgressService {
  ...
  private initializeProgress(): void {
    ...
    const initialProgress: UserProgress = {
      ...
      streaks: {
        ...
        milestones: [{ days: 7 }, { days: 30 }, { days: 100 }, { days: 365 }],
      },
      achievements: [],
    };
  ...
  recordAssessmentCompletion(
    ...
    const previousAssessment = progress.assessmentHistory
      ...
      .sort(
        (a: AssessmentProgress, b: AssessmentProgress) =>
    ...
    if (assessmentProgress.improvement && assessmentProgress.improvement > 0) {
      progress.stats.assessmentImprovementRate = this.calculateImprovementRate(
  ...
  recordExerciseSession(
    ...
    let exerciseProgress = progress.exerciseProgress.find(
      (ep: ExerciseProgress) => ep.exerciseId === exerciseId,
    );
    ...
    exerciseProgress.averageRating = this.calculateAverageRating(
      exerciseProgress.completedSessions,
    );
    ...
    progress.stats.averageExerciseRating = this.calculateGlobalExerciseRating(
  ...
  startLifestyleTip(tipId: string, tipTitle: string, tipType: string, totalSteps: number): void {
    ...
    const existingTip = progress.lifestyleTipProgress.find(
      (ltp: LifestyleTipProgress) => ltp.tipId === tipId,
    );
  ...
  updateLifestyleTipProgress(
    ...
    const tipProgress = progress.lifestyleTipProgress.find(
      (ltp: LifestyleTipProgress) => ltp.tipId === tipId,
    );
    ...
    if (tipProgress.progressPercentage >= 100) {
      ...
      progress.stats.tipCompletionRate =
        (progress.stats.totalTipsCompleted / progress.stats.totalTipsStarted) * 100;
    }
  ...
  startReadingArticle(articleId: string, articleTitle: string, articleType: string): void {
    ...
    const existingArticle = progress.articleProgress.find(
      (ap: ArticleProgress) => ap.articleId === articleId,
    );
  ...
  completeArticleReading(
    ...
    const articleProgress = progress.articleProgress.find(
      (ap: ArticleProgress) => ap.articleId === articleId,
    );
  ...
  updateGoalProgress(goalType: GoalType, increment: number): void {
    ...
    const activeGoals = progress.goals.filter(
      (g: UserGoal) => g.type === goalType && g.status === GoalStatus.ACTIVE,
    );
  ...
  toggleBookmark(type: 'exercise' | 'tip' | 'article', id: string): void {
    ...
    switch (type) {
      case 'exercise': {
        const exercise = progress.exerciseProgress.find(
          (ep: ExerciseProgress) => ep.exerciseId === id,
        );
        if (exercise) exercise.isBookmarked = !exercise.isBookmarked;
        break;
      }
      case 'tip': {
        const tip = progress.lifestyleTipProgress.find(
          (ltp: LifestyleTipProgress) => ltp.tipId === id,
        );
  ...
  private checkAchievements(): void {
    ...
    if (progress.stats.totalAssessments === 1) {
      this.unlockAchievement(
        'first_assessment',
        'Self-Aware',
        'Completed your first mental health assessment',
    ...
    if (progress.stats.totalExerciseSessions === 1) {
      this.unlockAchievement(
        'first_exercise',
        'Getting Started',
        'Completed your first wellness exercise',
    ...
    if (progress.stats.totalExerciseSessions === 10) {
      this.unlockAchievement(
        ...
        'Completed 10 exercise sessions',
        AchievementCategory.EXERCISE,
      );
    }
  ...
  private getUpcomingGoals(progress: UserProgress): UserGoal[] {
    return progress.goals
      ...
      .sort((a, b) => {
        const aDeadline = this.toDate(a.deadline);
        const bDeadline = this.toDate(b.deadline);
        ...
        const aCreatedAt = this.toDate(a.createdAt)?.getTime() ?? 0;
        const bCreatedAt = this.toDate(b.createdAt)?.getTime() ?? 0;
  ...
  private calculateImprovementRate(assessments: AssessmentProgress[]): number {
    const improvements = assessments.filter(
      (a: AssessmentProgress) => a.improvement && a.improvement > 0,
    );
  ...
  private getFavoriteExerciseType(exercises: ExerciseProgress[]): string {
    ...
    exercises.forEach((exercise) => {
      typeCounts[exercise.exerciseType] =
        (typeCounts[exercise.exerciseType] || 0) + exercise.totalSessions;
    });
  ...
  getActiveGoals(): UserGoal[] {
    return (
      this.userProgressSubject.value?.goals.filter(
        (g: UserGoal) => g.status === GoalStatus.ACTIVE,
      ) || []
    );
  }
}
