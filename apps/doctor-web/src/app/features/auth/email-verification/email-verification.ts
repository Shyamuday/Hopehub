...
@Component({
  selector: 'app-provider-email-verification',
  imports: [RouterLink],
  template: `
    <main class="verify-page">
      <section class="verify-card">
        ...
        <h1>{{ title() }}</h1>
        <p>{{ message() }}</p>
        <a routerLink="/login" class="btn">Go to provider sign in</a>
      </section>
    </main>
  `,
  styles: [
...
export class ProviderEmailVerification {
  private readonly http = inject(HttpClient);
  private readonly route = inject(ActivatedRoute);
  private readonly apiBase = environment.apiUrl;

  readonly title = signal('Verifying your provider email…');
  readonly message = signal('Please wait while we confirm your HopeHub provider email.');
