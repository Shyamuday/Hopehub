...
export class CaseAnalysisApiService {
  ...
  suggestRubrics(q: string, sourceId?: string) {
    return firstValueFrom(
      this.http.get<{ suggestions: RubricSuggestion[] }>(
        `${this.apiBase}${API_PATHS.DOCTOR.REPERTORY_RUBRICS_SUGGEST}`,
        {
          params: {
            q,
            ...(sourceId ? { sourceId } : {}),
  ...
  loadOrCreatePracticeSession() {
    return firstValueFrom(
      this.http.get<{ analysis: CaseAnalysis }>(
        `${this.apiBase}${API_PATHS.DOCTOR.REPERTORY_PRACTICE_SESSION}`,
      ),
  ...
  updateAnalysis(
    ...
    return firstValueFrom(
      this.http.patch<{ analysis: CaseAnalysis }>(
        `${this.apiBase}${API_PATHS.DOCTOR.CASE_ANALYSIS(analysisId)}`,
        payload,
      ),
    ).then((response) => response.analysis);
  }
  ...
  repertorize(analysisId: string) {
    return firstValueFrom(
      this.http.post<{ analysis: CaseAnalysis }>(
        `${this.apiBase}${API_PATHS.DOCTOR.CASE_ANALYSIS_REPERTORIZE(analysisId)}`,
        {},
      ),
    ).then((response) => response.analysis);
  }
  ...
  selectRemedy(analysisId: string, remedyId: string, overrideRationale?: string | null) {
    return firstValueFrom(
      this.http.patch<{ analysis: CaseAnalysis }>(
        `${this.apiBase}${API_PATHS.DOCTOR.CASE_ANALYSIS_SELECT_REMEDY(analysisId)}`,
        {
          remedyId,
  ...
  loadMateriaMedica(
    ...
    return firstValueFrom(
      this.http.get<MateriaMedicaResponse>(
        `${this.apiBase}${API_PATHS.DOCTOR.REPERTORY_REMEDY_MATERIA_MEDICA(remedyId)}`,
        {
          params: {
  ...
  uploadClinicalMedia(
    ...
    const formData = this.clinicalMediaFormData(payload);
    return firstValueFrom(
  ...
  private clinicalMediaFormData(payload: {
    mediaType: string;
    bodyRegion?: string;
    observations?: string;
    patientConsent: boolean;
    ...
    formData.append('patientConsent', String(payload.patientConsent));
    for (const [key, value] of Object.entries(payload)) {
      if (key !== 'file' && key !== 'mediaType' && key !== 'patientConsent' && value) {
  ...
  suggestClinicalMediaPhrases(payload: {
    ...
    return firstValueFrom(
      this.http.post<{ phrases: string[] }>(
        `${this.apiBase}${API_PATHS.DOCTOR.CLINICAL_MEDIA_SUGGEST_PHRASES}`,
        payload,
      ),
