...
export class AppointmentsPrescriptionsService {
  ...
  savePrescription(
    ...
    if (prescriptionId) {
      return firstValueFrom(
        this.http.put(
          `${this.apiBase}${API_PATHS.DOCTOR.PRESCRIPTIONS}/${prescriptionId}`,
          payload,
        ),
      );
    }
  ...
  loadTemplates() {
    return firstValueFrom(
      this.http.get<{ templates: PrescriptionTemplate[] }>(
        `${this.apiBase}${API_PATHS.DOCTOR.PRESCRIPTION_TEMPLATES}`,
      ),
  ...
  deleteTemplate(id: string) {
    return firstValueFrom(
      this.http.delete(`${this.apiBase}${API_PATHS.DOCTOR.PRESCRIPTION_TEMPLATES}/${id}`),
    );
  }
