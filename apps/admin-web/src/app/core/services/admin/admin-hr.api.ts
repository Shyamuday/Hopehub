...
export class AdminHrApi extends AdminApiBase {
  ...
  getHrDoctors() {
    return firstValueFrom(
      this.http.get<{ doctors: Array<any> }>(`${this.apiBase}${API_PATHS.HR.DOCTORS}`),
    );
  }

  getHrDoctor(id: string) {
    return firstValueFrom(
      this.http.get<{ doctor: any }>(`${this.apiBase}${API_PATHS.HR.DOCTORS}/${id}`),
    );
  }

  updateHrDoctor(id: string, data: Record<string, unknown>) {
    return firstValueFrom(
      this.http.put<{ doctor: any }>(`${this.apiBase}${API_PATHS.HR.DOCTORS}/${id}`, data),
    );
  }

  generateDoctorLetter(id: string, clinicName?: string, clinicAddress?: string) {
    return firstValueFrom(
      this.http.post<{ letter: any }>(`${this.apiBase}${API_PATHS.HR.DOCTORS}/${id}/letter`, {
        clinicName,
        clinicAddress,
      }),
    );
  ...
  getHrUsers() {
    return firstValueFrom(
      this.http.get<{ hrUsers: any[] }>(`${this.apiBase}${API_PATHS.HR.USERS}`),
    );
  }

  setHrUserStatus(id: string, isActive: boolean) {
    return firstValueFrom(
      this.http.patch(`${this.apiBase}${API_PATHS.HR.USERS}/${id}/status`, { isActive }),
    );
  }

  getDoctorLetter(id: string) {
    return firstValueFrom(
      this.http.get<{ letter: any }>(`${this.apiBase}${API_PATHS.HR.DOCTORS}/${id}/letter`),
    );
  }
  ...
  getHrUserStores(hrUserId: string) {
    return firstValueFrom(
      this.http.get<{ assigned: any[]; all: any[] }>(
        `${this.apiBase}${API_PATHS.HR.USERS}/${hrUserId}/stores`,
      ),
    );
  }
  ...
  grantHrStoreAccess(hrUserId: string, storeId: string) {
    return firstValueFrom(
      this.http.post(`${this.apiBase}${API_PATHS.HR.USERS}/${hrUserId}/stores`, { storeId }),
    );
  }

  revokeHrStoreAccess(hrUserId: string, storeId: string) {
    return firstValueFrom(
      this.http.delete(`${this.apiBase}${API_PATHS.HR.USERS}/${hrUserId}/stores/${storeId}`),
    );
  }

  grantAllStores(hrUserId: string) {
    return firstValueFrom(
      this.http.post(`${this.apiBase}${API_PATHS.HR.USERS}/${hrUserId}/stores/all`, {}),
    );
  }
  ...
  getHrEmployees(params: { q?: string; type?: string; status?: string }) {
    return firstValueFrom(
      this.http.get<{ employees: Array<any>; total: number }>(
        `${this.apiBase}${API_PATHS.HR.EMPLOYEES}`,
        {
          params: {
            q: params.q ?? '',
            type: params.type ?? FILTER_ALL,
  ...
  updateHrStoreStaff(id: string, data: Record<string, unknown>) {
    return firstValueFrom(
      this.http.put<{ staff: any }>(`${this.apiBase}${API_PATHS.HR.STORE_STAFF}/${id}`, data),
    );
  }

  generateStoreStaffLetter(id: string) {
    return firstValueFrom(
      this.http.post<{ letter: any }>(
        `${this.apiBase}${API_PATHS.HR.STORE_STAFF}/${id}/letter`,
        {},
      ),
    );
  }
  ...
  getStoreStaffLetter(id: string) {
    return firstValueFrom(
      this.http.get<{ letter: any }>(`${this.apiBase}${API_PATHS.HR.STORE_STAFF}/${id}/letter`),
    );
  }

  setDoctorAssignment(id: string, data: { isOnline: boolean; clinicStoreId?: string | null }) {
    return firstValueFrom(
      this.http.put(`${this.apiBase}${API_PATHS.HR.DOCTORS}/${id}/assignment`, data),
    );
  }
  ...
  getAdminLeaves(params: { status?: string; empType?: string; page?: number; pageSize?: number }) {
    return firstValueFrom(
      this.http.get<{ leaves: Array<any>; total: number }>(
        `${this.apiBase}${API_PATHS.HR.LEAVES}`,
        {
          params: {
            status: params.status ?? FILTER_ALL,
  ...
  createAdminLeave(data: any) {
    return firstValueFrom(
      this.http.post<{ leave: any }>(`${this.apiBase}${API_PATHS.HR.LEAVES}`, data),
    );
  }

  updateAdminLeave(id: string, data: { status: string; hrNote?: string }) {
    return firstValueFrom(
      this.http.patch<{ leave: any }>(`${this.apiBase}${API_PATHS.HR.LEAVES}/${id}`, data),
    );
  }
  ...
  getAdminStores() {
    return firstValueFrom(
      this.http.get<{ stores: Array<any> }>(`${this.apiBase}${API_PATHS.HR.STORES}`),
    );
  }

  createAdminStore(data: { name: string; code: string; address?: string; phone?: string }) {
    return firstValueFrom(
      this.http.post<{ store: any }>(`${this.apiBase}${API_PATHS.HR.STORES}`, data),
    );
  }

  getAdminStore(storeId: string) {
    return firstValueFrom(
      this.http.get<{ store: any }>(`${this.apiBase}${API_PATHS.HR.STORES}/${storeId}`),
    );
  }
  ...
  createAdminManager(storeId: string, data: any) {
    return firstValueFrom(
      this.http.post<{ manager: any }>(
        `${this.apiBase}${API_PATHS.HR.STORES}/${storeId}/managers`,
        data,
      ),
    );
  }
  ...
  createAdminStoreStaff(storeId: string, data: any) {
    return firstValueFrom(
      this.http.post<{ staff: any }>(
        `${this.apiBase}${API_PATHS.HR.STORES}/${storeId}/staff`,
        data,
      ),
    );
  }
  ...
  setAdminStoreStaffStatus(id: string, data: any) {
    return firstValueFrom(
      this.http.patch(`${this.apiBase}${API_PATHS.HR.STORE_STAFF}/${id}/status`, data),
    );
  }
  ...
  getAdminConsultations(params: {
    ...
    return firstValueFrom(
      this.http.get<{ consultations: any[]; total: number }>(
        `${this.apiBase}/admin/consultations`,
        {
          params: {
  ...
  updateHopeHubPackageUsage(consultationId: string, usedSessions: number) {
    return firstValueFrom(
      this.http.patch<{ consultation: any; packageUsage: any }>(
        `${this.apiBase}/admin/consultations/${consultationId}/hope-hub-usage`,
        { usedSessions },
      ),
    );
  }
  ...
  getPayroll(month: string) {
    return firstValueFrom(
      this.http.get<{ month: string; rows: Array<any>; summary: any }>(
        `${this.apiBase}${API_PATHS.HR.PAYROLL}`,
        {
          params: { month },
        },
      ),
  ...
  getSalaryEmployees(params: { q?: string; type?: string }) {
    return firstValueFrom(
      this.http.get<{ employees: Array<any> }>(
        `${this.apiBase}${API_PATHS.ADMIN.SALARY_EMPLOYEES}`,
        {
