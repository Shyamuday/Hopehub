...
export class AdminFinanceApi extends AdminApiBase {
  ...
  getRevenueTrend(months = 6) {
    return firstValueFrom(
      this.http.get<{ rows: Array<any> }>(
        `${this.apiBase}${API_PATHS.ADMIN.FINANCE.REVENUE_TREND}`,
        {
  ...
  getRevenueByDoctor(month?: string) {
    return firstValueFrom(
      this.http.get<{ rows: Array<any> }>(
        `${this.apiBase}${API_PATHS.ADMIN.FINANCE.REVENUE_BY_DOCTOR}`,
        {
          params: month ? { month } : {},
        },
      ),
  ...
  getRevenueByDisease(month?: string) {
    return firstValueFrom(
      this.http.get<{ rows: Array<any> }>(
        `${this.apiBase}${API_PATHS.ADMIN.FINANCE.REVENUE_BY_DISEASE}`,
        {
          params: month ? { month } : {},
        },
      ),
  ...
  getOutstandingPayments() {
    return firstValueFrom(
      this.http.get<{ payments: Array<any> }>(
        `${this.apiBase}${API_PATHS.ADMIN.FINANCE.OUTSTANDING}`,
      ),
    );
  }
  ...
  getPayslip(type: string, id: string, month: string) {
    return firstValueFrom(
      this.http.get<{ payslip: any }>(
        `${this.apiBase}${API_PATHS.ADMIN.FINANCE.PAYSLIP(type, id)}`,
        {
          params: { month },
        },
      ),
  ...
  getExpenses(params: {
    ...
    return firstValueFrom(
      this.http.get<{ expenses: Array<any> }>(
        `${this.apiBase}${API_PATHS.ADMIN.FINANCE.EXPENSES}`,
        {
          params: {
            level: params.level ?? '',
  ...
  getExpenseSummary(month: string) {
    return firstValueFrom(
      this.http.get<any>(`${this.apiBase}${API_PATHS.ADMIN.FINANCE.EXPENSES_SUMMARY}`, {
        params: { month },
      }),
    );
  }
  ...
  createExpense(data: Record<string, unknown>) {
    return firstValueFrom(
      this.http.post<{ expense: any }>(`${this.apiBase}${API_PATHS.ADMIN.FINANCE.EXPENSES}`, data),
    );
  }

  updateExpense(id: string, data: Record<string, unknown>) {
    return firstValueFrom(
      this.http.put<{ expense: any }>(
        `${this.apiBase}${API_PATHS.ADMIN.FINANCE.EXPENSES}/${id}`,
        data,
      ),
    );
  }
  ...
  deleteExpense(id: string) {
    return firstValueFrom(
      this.http.delete(`${this.apiBase}${API_PATHS.ADMIN.FINANCE.EXPENSES}/${id}`),
    );
  }
  ...
  getProviderPayouts(params: { status?: string; from?: string; to?: string; doctorId?: string }) {
    return firstValueFrom(
      this.http.get<any>(`${this.apiBase}${API_PATHS.ADMIN.FINANCE.PROVIDER_PAYOUTS}`, {
        params: {
  ...
  updateProviderPayout(
    id: string,
    data: { status: 'PENDING' | 'HOLD' | 'PAID'; payoutReference?: string; payoutNote?: string },
  ) {
    return firstValueFrom(
      this.http.patch<{ earning: any }>(
        `${this.apiBase}${API_PATHS.ADMIN.FINANCE.PROVIDER_PAYOUT(id)}`,
        data,
      ),
    );
  }
  ...
  backfillProviderPayouts() {
    return firstValueFrom(
      this.http.post<any>(`${this.apiBase}${API_PATHS.ADMIN.FINANCE.PROVIDER_PAYOUT_BACKFILL}`, {}),
    );
  }
