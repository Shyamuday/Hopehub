...
export class EmployeesPage implements OnInit {
  ...
  load(): void {
    ...
    this.api
      ...
      .catch(() => {
        this.loading.set(false);
        this.error.set('Could not load employees. Check your connection and try again.');
      });
  }
  ...
  async saveProfile(): Promise<void> {
    ...
    try {
      ...
      this.employees.update((list) =>
        list.map((e) => (e.id === this.selected().id ? { ...e, ...payload } : e)),
      );
  ...
  openLetter(): void {
    ...
    if (this.stores().length === 0) {
      ...
      this.api
        .getAdminStores()
        .then((r) => {
          this.stores.set(r.stores);
          this.storesLoading.set(false);
        })
        .catch(() => this.storesLoading.set(false));
    }
  }
